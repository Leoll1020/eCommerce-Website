...
<?php
$zip_array=array();
$result_1=$conn->query("SELECT zip_code FROM tax");
while ($crow_1=$result_1->fetch(PDO::FETCH_ASSOC)){
  $zip=$crow_1['zip_code'];
  $zip_array[]=$zip;
}
?>
...
<hearder>
    <h1  style="width: 1000px; margin: 50px auto 0 auto;">BEST Electronics</h1>

  <nav  style="width: 1000px; margin: 50px auto 0 auto;"><ul>
...
<div style="width: 800px; margin: 50px auto 0 auto;">
  <form name="order_form"  method="POST">
    <p><?php echo $pName ?></p>
    <p><?php echo "$" . $pPrice ?></p>
    <p>Your full name: (AJAX enabled)</p><input type="text" id="fname" name="fname" required>
    ...
    <p>Zipcode: (AJAX enabled)</p><input type="text" id="zipcode" name="zipcode" required>
    <p>Total Price:</p><input type="text" id="total_price" onKeyUp="doMath();" name="total_price" required>
    <p id ="total_prices">Total Price:</p>
    <button type="button" onclick="doMath()">Calculate Total Cost</button>
    <li id="cc">
...
<link rel="stylesheet" href="http://code.jquery.com/ui/1.11.4/themes/smoothness/jquery-ui.css">
  ...
  <script type="text/javascript">
  $(function() {
    var availableTags_1 = <?php echo json_encode($zip_array); ?>;
